	...
	if (strtoupper($local_signValue) == strtoupper($_REQUEST['signValue'])) {
		...
		if($ErrorCode == 20061){
			...
			if($_REQUEST['payment_status'] == 1 ){
				//支付成功
				$new_history->changeIdOrderState((int)Configuration :: get('OP_GIROPAY_SUCCEED_STATES'), $_REQUEST['order_number']);
				...
				if($_REQUEST['payment_authType'] == 1){
					...
					$new_history->changeIdOrderState((int)Configuration :: get('OP_GIROPAY_PENDING_STATES'), $_REQUEST['order_number']);
				}
			}else{
				//支付失败
				$new_history->changeIdOrderState((int)Configuration :: get('OP_GIROPAY_FAIL_STATES'), $_REQUEST['order_number']);
			}
		}
